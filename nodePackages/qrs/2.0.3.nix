...
buildNodePackage {
  ...
  # This preinstall step tries to npm install istanbul
  prePatch = ''
    python <<PATCH
    import json
    p_j = json.load(open("package.json"))
    p_j["scripts"].pop("preinstall")

    open("package.json", "w").write(json.dumps(p_j, indent=2))
    PATCH
  '';
